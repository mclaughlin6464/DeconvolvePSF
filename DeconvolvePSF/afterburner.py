...
This module requires two arguments, the exposure to calculate PSFs on and the output directory to store the temporary
files and the results. Afterburner will attempt to create a sub directory in that output directory for this
individual exposure, to prevent confusion with other modules.
'''
...
#TODO define verbose flag
#TODO split into a few smaller functions?

def afterburner(expid, outputDir):
    ...
    for hdulist in meta_hdulist:
        ...
        hdulist.writeto(outputDir+'_'.join(original_fname_split), clobber = True)
    ...
    command_list = [psfex_path, outputDir+'*.fits', "-c", psfex_config]
    ...
    np.save(outputDir+'%s_stars.npy'%expid, np.array(stars))
...
if __name__ == '__main__':
    ...
    afterburner(args['expid'], args['outputDir'])
